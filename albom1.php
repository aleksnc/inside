...
include('db.php'); ?>
<head>
<title>АЛЬБОМ: В ОЖИДАНИИ ЧУДА</title>
<style>
#menu ul ul{
display:block;
}
	...
	<div id='all'>
		<div id='content'>
			<div id="scrollbar1">
				...
				<?php
				$sel="SELECT COUNT(id) FROM inside where nomer='1' and type='img'";
				$my_qu = mysql_query($sel);
				$row = mysql_fetch_row($my_qu);
				$rov = $row[0];
				if($rov>0){
					$q ="SELECT * FROM inside where nomer='1' and type='img'";
					$rez = mysql_query($q);
					for($im=0;$im<$rov;$im++){
...
<div id='text'>
						<?php			
$q ="SELECT * FROM inside where nomer='1' and type='text'";
$rez = mysql_query($q);
$array = mysql_fetch_array($rez);
if(isset( $array['id'])){
	$content=$array['content'];
