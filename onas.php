			...
			<div id='menu'>		
				...
				<li  class="List1"><a  href='kompany.php'>О КОМПАНИИ</a></li>
					<li  class="List1"><a   style='color:#b98151;' href='index.php'>ПОРТФОЛИО</a>
						<ul >
							<li ><a style='color:#b98151;' href='onas.php'>о нас</a></li>
...
<div id='a_all'>
	<div id='all'>
		<div id='content'>
			<div id="scrollbar1">
				...
				<?php
				$sel="SELECT COUNT(id) FROM inside where type='img' and razdel='О НАС' and chernovik='0'";
				$my_qu = mysql_query($sel);
				$row = mysql_fetch_row($my_qu);
				$rov = $row[0];
				if($rov>0){
					$q ="SELECT * FROM inside where type='img' and razdel='О НАС' and chernovik='0'";
					$rez = mysql_query($q);
					for($im=0;$im<$rov;$im++){
	...
	<img style="height: 100px; width: 151px; " src="uploads/non.jpg" /></a>';}
						...
						<div id='text'>
						<?php			
$q ="SELECT * FROM inside where razdel='О НАС' and chernovik='0' and type='text' ";
$rez = mysql_query($q);
$array = mysql_fetch_array($rez);
if(isset( $array['id'])){
	$content=$array['content'];
