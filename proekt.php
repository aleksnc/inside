				...
				<li  class="List1"><a  href='kompany.php'>О КОМПАНИИ</a></li>
					<li  class="List1"><a style='color:#b98151;' href='index.php'>ПОРТФОЛИО</a>
						<ul >
							...
							<li ><a  style='color:#b98151;' href='proekt.php'>проектирование</a></li>
						</ul>
					</li>
			...
			<div id="scrollbar1">
				...
				<div class="viewport">
					<div class="overview">
						<div id='album'>
										<?php
				$sel="SELECT COUNT(id) FROM inside where type='img' and razdel='ПРОЕКТЫ'";
				$my_qu = mysql_query($sel);
				$row = mysql_fetch_row($my_qu);
				$rov = $row[0];
				if($rov>0){
					$q ="SELECT * FROM inside where type='img' and razdel='ПРОЕКТЫ'";
					$rez = mysql_query($q);
					for($im=0;$im<$rov;$im++){
...
<div id='text'>
						<?php			
$q ="SELECT * FROM inside where razdel='ПРОЕКТЫ' and chernovik='0' and type='text' ";
$rez = mysql_query($q);
$array = mysql_fetch_array($rez);
if(isset( $array['id'])){
	$content=$array['content'];
